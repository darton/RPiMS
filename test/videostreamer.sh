...
function stream_on {
    ...
    if ([ ! -n "$ffmpeg_pid" ]) then
        ...
        if [ ! -e /var/www/html/streaming/hls.js ]; then
            ...
            #/usr/bin/ffmpeg -y -hide_banner -use_wallclock_as_timestamps 1 -input_format h264 -f video4linux2 -video_size 640x480 -framerate 30 -i /dev/video0 -map_metadata 0 -metadata:s:v rotate="90" -c:v copy -f hls -hls_time 1 -hls_list_size 30 -hls_flags delete_segments        #/usr/bin/ffmpeg -y -hide_banner -use_wallclock_as_timestamps 1 -input_format h264 -f video4linux2  -video_size 640x480 -framerate 30 -i /dev/video0 -c:v copy -f hls -hls_time 6 -hls_list_size 6 -hls_flags delete_segments /dev/shm/streaming/live.m3u8
            ...
            raspivid -n -o - -t 0 -fps 25 -b 6000000 -rot 90 -w 640 -h 480 | /usr/bin/ffmpeg -y -hide_banner -use_wallclock_as_timestamps 1 -f h264 -i - -c:v copy -f hls -hls_time 6 -hls_list_size 30 -hls_flags delete_segments /dev/shm/streaming/live.m3u8
    else
